...
// (Q4) verificar se o grafo, passado como parâmetro de entrada, possui todos os nós com grau igual a k - int testek(TG *g, int k); e
...
int main(void){
    ...
    for(int i=0; i<10; i++){
        g = ins(g, i);
        add_ar_dir(g, i, 0);
        // add_ar_dir(g, i, (i*2+1)%10);
        // add_ar_dir(g, i, (i*3+5)%10);
        // add_ar_dir(g, i, (i*4+1)%10);
    }
    imp(g);

    printf("Todos os nós possuem tamanho 1?: %s\n", (testek(g, 1)) ? "sim" : "não");
