...
NO *divida(NO *p, int i, NO *e, int t){

  // Aloca o nó da direita
  NO *d = cria(t);
  d->n = t-1;
  d->folha = e->folha;
  
  // Copia os dados do nó da esquerda (antigo pai) para o da direita
  int j;
  for(j=0; j<t-1; j++) 
    d->chaves[j] = e->chaves[j+t];
  if(!e->folha){
    for(j=0; j<t; j++){
      d->filhos[j] = e->filhos[j+t];
      e->filhos[j+t] = NULL;
    }
  }
  e->n = t-1;

  // Arruma chaves que estejam lá como lixo e adiciona filho no pai
  for(j=p->n; j>=i; j--) 
  ...
  // Adiciona elemento no pai
  p->chaves[i-1] = e->chaves[t-1];
  p->n++;
...
NO *insere_incompleto(NO *x, int v, int t){
  ...
  // Se for folha só chega os elementos para o lado e adiciona no local apropriado
  if(x->folha){
  ...
  // Se não for folha procura o local apropriado
  while((i>=0) && (v<x->chaves[i])) i--;
  i++;

  // Se ele tiver o máximo de elementos divide
  if(x->filhos[i]->n == ((2*t)-1)){
    x = divida(x, (i+1), x->filhos[i], t);
    if(v>x->chaves[i]) i++;
  }

  // Chama a função de inserção no filho apropriado
