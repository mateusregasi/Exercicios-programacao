...
select 
    ...
    di.nome as divisao,
    round(sum(ven.valor)/count(ven.valor), 2) as "Media Salarial",
    max(ven.valor) as "Maior Salarial",
    min(ven.valor) as "Menor Salarial"
from 
    departamento as dep join divisao as di
        on (dep.cod_dep = di.cod_dep)
    join empregado as emp
        on (emp.lotacao_div = di.cod_divisao)
    natural join emp_venc as empv
    join vencimento as ven
        on (ven.cod_venc = empv.cod_venc)
group by dep.nome, di.nome
order by sum(ven.valor)/count(ven.valor) desc;
